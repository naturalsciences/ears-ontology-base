...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:include href="xslt/swrl-module.xsl" />
    <xsl:include href="xslt/common-functions.xsl"/>
    <xsl:include href="xslt/structural-reasoner.xsl"/>
    ...
    <xsl:variable name="default-labels" select="document(concat('xslt/',$def-lang,'.xml'))" />
    <xsl:variable name="labels" select="document(concat('xslt/',$lang,'.xml'))" />
    ...
    <xsl:template name="structure">
        ...
        <body>
            <div class="head">
                ...
                <dl>
                    <dt>
                        <xsl:value-of select="f:getDescriptionLabel('visualisation')" />:</dt>
                    <dd>
                        <a href="http://ontologies.ef-ears.eu/ears2/1/current/earsv2-onto.rdf">
            ...
            <p class="endnote">
                ...
                <xsl:text> </xsl:text>
                <a href="http://www.essepuntato.it">Silvio Peroni</a>.</p>
        </body>
    </xsl:template>
    ...
    <xsl:template match="skos:definition[f:isInLanguage(.)]">
        <!--<dt><xsl:value-of select="f:getDescriptionLabel('definition')" />:</dt>-->
        <dt>Definition:</dt>
        <div class="comment">
            <dd>
                <xsl:call-template name="get.content" />
    ...
    <xsl:template match="dc:date|dcterms:date[ancestor::owl:Ontology]">
        <dt>
            <xsl:value-of select="f:getDescriptionLabel('date')" />:</dt>
        <dd>
            <xsl:choose>
    ...
    <xsl:template match="owl:versionInfo[f:isInLanguage(.)]">
        <dt>
            <xsl:value-of select="f:getDescriptionLabel('currentversion')" />:</dt>
        <dd>
            <xsl:apply-templates />
        </dd>
    </xsl:template>
    ...
    <xsl:template match="owl:priorVersion | owl:backwardCompatibleWith | owl:incompatibleWith">
        ...
        <dt>
            <xsl:value-of select="f:getDescriptionLabel($versionLabel)" />:</dt>
        <dd>
            <xsl:choose>
    ...
    <xsl:template match="skos:prefLabel[f:isInLanguage(.)]">
        <!--<dt><xsl:value-of select="f:getDescriptionLabel('label')" />:</dt>-->
    ...
    <xsl:template match="/rdf:RDF/rdf:Description[exists(rdf:type[@*:resource = 'http://www.w3.org/2002/07/owl#AllDisjointClasses'])]">
        <div id="{generate-id()}" class="entity">
            <h3>
                <xsl:value-of select="f:getDescriptionLabel('disjointclasses')" />
                <xsl:text> </xsl:text>
                <xsl:call-template name="get.backlink" />
            </h3>
            <p>
    ...
    <xsl:template match="/rdf:RDF/owl:Restriction[exists(rdfs:subClassOf)]">
        <div id="{generate-id()}" class="entity">
            <h3>
                <xsl:value-of select="f:getDescriptionLabel('subclassdefinition')" />
                <xsl:text> </xsl:text>
                <xsl:call-template name="get.backlink" />
            </h3>
            <p>
                ...
                <strong>
                    <xsl:text> </xsl:text>
                    <xsl:value-of select="f:getDescriptionLabel('issubclassof')" />
                </strong>
            </p>
    ...
    <xsl:template match="/rdf:RDF/owl:Restriction[exists(owl:equivalentClass)]">
        <div id="{generate-id()}" class="entity">
            <h3>
                <xsl:value-of select="f:getDescriptionLabel('equivalentdefinition')" />
                <xsl:text> </xsl:text>
                <xsl:call-template name="get.backlink" />
            </h3>
            <p>
    ...
    <xsl:template name="get.imports">
        <xsl:if test="exists(owl:imports)">
            <dl>
                <dt>
                    <xsl:value-of select="f:getDescriptionLabel('importedontologies')" />:</dt>
        ...
        <xsl:if test="exists(dc:creator | dc:contributor | dcterms:creator[ancestor::owl:Ontology] | dcterms:contributor[ancestor::owl:Ontology])">
            <dl>
                <xsl:if test="exists(dc:creator|dcterms:creator[ancestor::owl:Ontology])">
                    <dt>
                        <xsl:value-of select="f:getDescriptionLabel('authors')" />:</dt>
                ...
                <xsl:if test="exists(dc:contributor|dcterms:contributor[ancestor::owl:Ontology])">
                    <dt>
                        <xsl:value-of select="f:getDescriptionLabel('contributors')" />:</dt>
    ...
    <xsl:template name="get.publisher">
        <xsl:if test="exists(dc:publisher | dcterms:publisher)">
            <dl>
                <dt>
                    <xsl:value-of select="f:getDescriptionLabel('publisher')" />:</dt>
    ...
    <xsl:template name="get.characteristics">
        ...
        <xsl:if test="exists($nodes)">
            <p>
                <strong>
                    <xsl:value-of select="f:getDescriptionLabel('hascharacteristics')" />:</strong>
                <xsl:text> </xsl:text>
